...
class ExportPdfReport 
{
    ...
    array(
        'url'=>'neteye/monitoring/list/services?service_state=2',
        'type'=>'service'
    ),
    array(
        'url'=>'neteye/monitoring/list/hosts?host_state=1',
        'type'=>'host'
    ),
    array(
        'url'=>'neteye/monitoring/list/services?service_state=1',
        'type'=>'service'
    ),
    array(
        'url'=>'neteye/monitoring/list/hosts?host_state=0',
        'type'=>'host'
    ),
);
            ...
            if (!$this->isHelpOption()) {
                ...
                foreach ($this->array_url as $url) {
                    if($url['url']==NULL){
                        ...
                        $baseUrl = sprintf(
                            '%s://%s%s/'. $url['url']  . '&limit='.$this->limit,
                            $this->getProtocol(),
                            $this->getDomain(),
                            $this->getPort()
                        );
